...
export const quartersService = {
  ...
  async closeService(
    ...
    await supabase
      ...
      .insert({
        ...
        author_id: authorId,
        author_role: 'system',
        message: closingMessage || 'Service request closed.',
        document_urls: [],
      });
  },

  async getEOProfile(userId: string): Promise<{ fullName: string; email: string } | null> {
    const { data, error } = await supabase
      .from('users')
    ...
    return { fullName: (data as any).full_name ?? '', email: (data as any).email ?? '' };
  },
  ...
  async declineAllotment(
    ...
    const now = new Date().toISOString();
    const nextRequestStatus = declineType === 'decline_cancel' ? 'WITHDRAWN' : 'SUBMITTED';
    const subStatus = declineType === 'decline_cancel' ? 'CANCELLED_AFTER_DECLINE' : 'DECLINED';
