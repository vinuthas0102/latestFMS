                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-3.5 py-2.5 rounded-xl text-xs font-medium border transition-all ${
                    timeSlot === slot
                      ...
                      : 'bg-white text-gray-600 border-gray-200 hover:border-sky-300 hover:bg-sky-50/40'
                  }`}
                >
                  <Clock size={13} className={timeSlot === slot ? 'text-white' : 'text-gray-400'} />
                  {slot}
                </button>
              ))}
            </div>
            ...
            <div className="space-y-2">
              {INSPECTORS.map(insp => (
                <button
                  key={insp.id}
                  type="button"
                  onClick={() => setInspectorId(insp.id)}
