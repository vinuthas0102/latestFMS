...
import { Calendar, Clock, User, FileText, X, Send, CheckSquare, Square, HardHat } from 'lucide-react';
...
const DISPATCH_TARGETS = [
  { id: 'emp', name: 'Employee (Allottee)', email: 'employee@gov.in' },
  { id: 'eo', name: 'Estate Office', email: 'estate.office@gov.in' },
  { id: 'maint', name: 'Maintenance Cell', email: 'maintenance@gov.in' },
];
...
export function InspectionFormModal({ requestRef, quarterRef, tr, onClose, onSubmit, submitting = false }: Props) {
  ...
  const handleSubmit = () => {
    ...
    onSubmit({ date, timeSlot, inspectorId, inspectorName: insp?.name ?? '', remarks, dispatchTargets: dispatchIds });
  };
    ...
    <div className="fixed inset-0 z-[4000] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden" style={{ maxHeight: '92vh' }}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-sky-200 mt-0.5 leading-tight">
                {[requestRef ?? tr?.id, quarterRef ?? q?.quarter_number].filter(Boolean).join(' · ') || 'Vacate Request'}
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1.5">Inspection Date <span className="text-red-500">*</span></label>
          ...
          <div>
            ...
            <div className="space-y-2">
              {DISPATCH_TARGETS.map(t => (
                <button key={t.id} type="button" onClick={() => toggleDispatch(t.id)}
                  className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl border transition-all ${dispatchIds.includes(t.id) ? 'bg-sky-50/60 border-sky-200' : 'bg-white border-gray-200 hover:border-gray-300'}`}>
                  {dispatchIds.includes(t.id) ? <CheckSquare size={15} className="text-sky-600 shrink-0" /> : <Square size={15} className="text-gray-300 shrink-0" />}
        ...
        <div className="shrink-0 bg-white border-t border-gray-100 px-6 py-4 flex items-center gap-3">
          ...
          <button onClick={handleSubmit} disabled={!canSubmit || submitting}
            ...
            {submitting ? (<><svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" /></svg>Scheduling…</>)
              : (<><Send size={15} />Submit</>)}
          </button>
        </div>
      </div>
...
export { buildDefaultChecklist } from '../../constants/inspectionChecklist';
export type { ChecklistItemDraft } from '../../constants/inspectionChecklist';
