...
import {
  ...
  Layers, RotateCcw, Shield, History,
} from 'lucide-react';
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-gray-50 via-white to-sky-50/30">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-3 pb-3">
          ...
          <div className={`relative ${welcomeInfo.gradient} rounded-2xl px-4 py-3 overflow-hidden shadow-lg`}>
            ...
            <div className="relative flex items-center justify-between gap-3">
              ...
              {/* Right: filter icon + view switcher + My Requests */}
              <div className="flex items-center gap-2 flex-shrink-0">

                {/* Filter icon button — works for all display types */}
                <button
                  onClick={() => {
                    const next = !filterDrawerOpen;
                    setFilterDrawerOpen(next);
                    // in list view, sync the persistent sidebar collapsed state
                    if (isListView) setSidebarCollapsed(!next);
                  }}
                  title="Search & Filter"
                  className={`relative p-2 rounded-xl border backdrop-blur-sm transition-all duration-200 shadow-sm hover:shadow-md ${
                    filterDrawerOpen || activeFilterCount > 0
                      ? 'bg-white text-gray-800 border-white/80'
                      : 'bg-white/20 text-white border-white/30 hover:bg-white/30 hover:border-white/50'
                  }`}
                >
                  <SlidersHorizontal size={16} />
                  {activeFilterCount > 0 && (
                    <span className="absolute -top-1.5 -right-1.5 bg-blue-600 text-white text-xs font-bold rounded-full w-4 h-4 flex items-center justify-center shadow-md leading-none">
                      {activeFilterCount}
                    </span>
                  )}
                </button>
                ...
                <div className="inline-flex items-center bg-white/20 backdrop-blur-sm rounded-lg border border-white/30 p-1 gap-0.5">
                  {VIEW_OPTIONS.map(({ mode, icon, label }) => (
                    <button
                      ...
                      onClick={() => {
                        setView(mode);
                        // when switching to list view, sync sidebar state with drawer
                        if (mode === 'list') {
                          setSidebarCollapsed(!filterDrawerOpen);
                        }
                      }}
                      title={label}
      ...
      <div className="flex-1 overflow-hidden relative">
        ...
        {/* ── Filter drawer backdrop (card + table views) ── */}
        {filterDrawerOpen && !isListView && (
          <div
        ...
        {/* ── Left filter drawer for card + table views (overlay) ── */}
        {!isListView && (
          <div
            className={`absolute left-0 top-0 h-full z-40 transition-transform duration-300 ease-in-out ${
              filterDrawerOpen ? 'translate-x-0' : '-translate-x-full'
            }`}
            style={{ width: '288px' }}
          >
            <div className="bg-white border-r border-gray-200 shadow-xl h-full flex flex-col overflow-hidden">
              <QuarterFilterSidebar
                collapsed={false}
                onCollapse={() => {}}
        ...
        <div className={`h-full overflow-y-auto`}>
          <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 ${isListView ? 'flex gap-0 items-start' : ''}`}>
            ...
            {/* Left sidebar (list view — persistent, collapsible) */}
            {isListView && (
              <div
                className={`flex-shrink-0 self-start sticky top-4 transition-all duration-300 ${sidebarCollapsed ? 'w-11' : 'w-72'}`}
              >
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-x-hidden flex flex-col" style={{ height: 'calc(100vh - 180px)' }}>
                  <QuarterFilterSidebar
                    ...
                    onCollapse={(v) => {
                      ...
                      // sync banner filter icon state with sidebar visibility
                      setFilterDrawerOpen(!v);
                    }}
                    filters={sidebarFilters}
                    onChange={(patch) => setSidebarFilters(prev => ({ ...prev, ...patch }))}
                    onClear={clearFilters}
